...
# Remove currently active loans (status "Current")
data <- data %>%
  filter(!grepl("Current", loan_status, ignore.case = TRUE))

# Keep only grade E loans (sub_grade starts with "E")
data <- data %>%
...
# ------------------------------
# 2. Variable selection and standardization
# ------------------------------
selected_vars <- data_clean %>%
  select(int_rate, loan_amnt, annual_inc, installment, dti)

# Convert character columns to numeric (remove % and commas)
...
# Standardize all selected variables (mean 0, sd 1)
...
# Add standardized variables to the main dataset
...
# ------------------------------
# 3. Construct survival dataset (event type and survival time)
# ------------------------------
survival_data <- data_final %>%
  mutate(
    ...
    # Indicator: current but delinquent
...
# Final analysis set (remove NA)
survival_final <- survival_data %>%
  filter(!is.na(survival_time), !is.na(u))

# Check event distribution (including defaults)
...
# Remove default events (u == 1) for this analysis (focus on prepayment vs cured)
survival_final <- survival_data %>%
  filter(!is.na(survival_time), !is.na(u), u != 1)

event_summary <- survival_final %>%
  count(u) %>%
  mutate(
    event_type = ifelse(u == 2, "Prepayment", "No event"),
    proportion = n / nrow(survival_final)
  )

cat("Event distribution after removing defaults:\n")
print(event_summary)
...
# ------------------------------
# 4. Build interval-censored data structure
# ------------------------------
library(survival)
...
zz <- cbind(z1, z2, z4)   # covariates for cure probability
xx <- cbind(z1, z2)       # covariates for survival time

# Initial parameter values (for reference, not used in EM directly)
gamma1 <- c(1, 1, 1)
gamma2 <- c(2, 2, 2)
a1 <- 2
beta1 <- c(1, 1.5)
...
# Function to create interval-censored data from observed event times
create_interval_censored_data <- function(survival_final, gamma1, gamma2, a1, beta1, b1, a2, beta2, b2, zz, xx){
  ...
  for (i in 1:N) {
    if (u[i] == 0) {               # Censored (no event)
      L[i] <- survival_time[i]
      R[i] <- Inf
      Status[i] <- 0
    } else if (u[i] == 2) {        # Prepayment event -> interval censored
  ...
  return(list(
    ...
    cure = cure, uncure2 = uncure2, censorrate = censorrate
  ))
}
...
# ------------------------------
# 5. EM algorithm for mixture cure model (only prepayment vs cured)
# ------------------------------
em_ic <- function(interval_censored_result) {
  ...
  while (convergence >= eps & iter <= emmax) {
    ...
    em_temp2 <- exp(cbind(zz) %*% gamma2hat) / (exp(cbind(zz) %*% gamma2hat) + 1)
    em_temp0 <- 1 - em_temp2
    xbeta2hat <- xx %*% beta2hat
    L <- ifelse(L <= 0, 1e-3, L)
    sur2 <- 1 - pweibull(L, shape = 1/b2hat, scale = exp(xbeta2hat + a2hat))
    sur0 <- em_temp0 + sur2 * em_temp2
    
    p10 <- Status * as.numeric(u == 0) + (1 - Status) * (em_temp0 / sur0)
    p12 <- Status * as.numeric(u == 2) + (1 - Status) * (em_temp2 * sur2 / sur0)
    
    # Negative log-likelihood for cure probability part (gamma2)
    lc1 <- function(gamma2, a2, b2, beta2) {
      temp2 <- exp(cbind(zz) %*% gamma2) / (exp(cbind(zz) %*% gamma2) + 1)
      temp0 <- 1 - temp2
      loglikelihood <- sum(as.numeric(u == 2) * log(temp2) + 
                             (1 - Status) * as.numeric(u == 2) * log(pmax(temp0, 1e-10)), na.rm = TRUE)
      return(-loglikelihood)
    }
    
    # Negative log-likelihood for survival part (Weibull AFT)
    lc2 <- function(a2, b2, beta2) {
      xbeta2 <- xx %*% beta2
      L <- ifelse(L <= 0, 1e-3, L)
      R <- ifelse(R == 0, 1e-3, R)
      ...
      dan[dan <= 0] <- 1e-10  
      log_dan <- log(dan)
      loglikelihood <- sum(Status * as.numeric(u == 2) * log_dan + 
                             (1 - Status) * as.numeric(u == 2) * log(pmax(S2L, 1e-10))) 
      return(-loglikelihood)
    }
    
    # M-step: update gamma2
    update_gamma2 <- optim(gamma2hat, lc1, a2 = a2hat, b2 = b2hat, beta2 = beta2hat,
                           method = "Nelder-Mead", control = list(reltol = 1e-4, maxit = 500))$par
    
    # M-step: update a2, beta2, b2 simultaneously
    ...
    convergence <- sum(abs(update_gamma2 - gamma2hat),
                       abs(update_a2 - a2hat),
  ...
  final_ll <- -lc1(gamma2hat, a2hat, b2hat, beta2hat) - lc2(a2hat, b2hat, beta2hat)
  k <- length(gamma2hat) + 1 + 1 + length(beta2hat)
  aic <- -2 * final_ll + 2 * k
  
  list(gamma2hat = gamma2hat,
...
results <- list(gamma2hat = gamma2hat,
                a2hat = a2hat, beta2hat = beta2hat, b2hat = b2hat,
                logLik = final_ll,
                params = k,
                AIC = aic)
...
# ------------------------------
# 6. Evaluate log-loss at multiple time points (9,18,27,36 months)
# ------------------------------
...
for (m in node_months) {
  ...
  # Create binary label: event (prepayment) before or at m months
  surv_m <- survival_final %>%
    mutate(
      time_m = pmin(survival_time, m),
      label_m = ifelse(survival_time <= m & u == 2, 1, 0)
    ) %>%
  ...
  cat("  Sample size:", N_m, ", Prepayment events:", n_events, "\n")
...
# ------------------------------
# 7. Bootstrap inference for parameter uncertainty
# ------------------------------
B <- 200
...
# Bootstrap estimates: mean, SE, percentile CI
...
# Wald test using original fit (single estimate) and bootstrap SE
original_fit <- em_ic(interval_censored_result)
theta_hat <- c(
  original_fit$gamma2hat,
...
# ------------------------------
# 8. Plot survival curves from bootstrap estimates
# ------------------------------
library(ggplot2)

# Survival function for event type 2 (prepayment)
...
xx <- cbind(1, 2)   # fixed covariate values for plotting
time <- seq(0, 80, by = 0.1)
survival_results <- matrix(0, nrow = 197, ncol = length(time))

for (i in 1:197) {
  a2hat <- boot_results[i, 4]
  b2hat <- boot_results[i, 7]
  beta2hat <- boot_results[i, 5:6]
  survival_results[i, ] <- sapply(time, function(t) weibull_survival(t, a2hat, b2hat, beta2hat, xx))
}

# Pointwise mean and 95% CI
lcil_surv <- apply(survival_results, 2, quantile, probs = 0.025)
ucil_surv <- apply(survival_results, 2, quantile, probs = 0.975)
est_surv <- sapply(time, function(t) weibull_survival(t, a2 = bootstrap_means[4],
                                                      b2 = bootstrap_means[7], beta2 = bootstrap_means[5:6], xx = xx))
...
ggsave(filename = file.path(save_path, "w.MCM2.S2.png"), plot = last_plot(), width = 4, height = 5, dpi = 300)

# ------------------------------
# 9. Overall survival probability (cured + prepayment)
# ------------------------------
library(ggplot2)

weibull_survival <- function(t, gamma2, a2, b2, beta2, xx, zz) {
  temp2 <- exp(cbind(zz) %*% gamma2) / (exp(cbind(zz) %*% gamma2) + 1)
  temp0 <- 1 - temp2
  xbeta2 <- xx %*% beta2
  S2 <- 1 - pweibull(t, shape = 1/b2, scale = exp(xbeta2 + a2))
...
for (i in 1:197) {
  gamma2hat <- boot_results[i, 1:3]
  a2hat <- boot_results[i, 4]
  b2hat <- boot_results[i, 7]
  beta2hat <- boot_results[i, 5:6]
  survival_results[i, ] <- sapply(time, function(t) weibull_survival(t, gamma2hat, a2hat, b2hat, beta2hat, xx, zz))
}

lcil_surv <- apply(survival_results, 2, quantile, probs = 0.025)
ucil_surv <- apply(survival_results, 2, quantile, probs = 0.975)
est_surv <- sapply(time, function(t) weibull_survival(t, gamma2 = bootstrap_means[1:3],
                                                      a2 = bootstrap_means[4], b2 = bootstrap_means[7], beta2 = bootstrap_means[5:6],
                                                      xx = xx, zz = zz))
...
ggsave(filename = file.path(save_path, "w.MCM2.S.png"), plot = last_plot(), width = 4, height = 5, dpi = 300)
